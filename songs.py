...
SONGS = [
    ...
    ("2026-03-31", "ultra魂",                    "ultra soul",             "lpg5nhWapjU"),
    ("2026-07-06", "イチジク煙",                 "Fig Smoke",              "4LDQXYTfWjA"),
]
...
for song_date, *song_data in SONGS:
    songs.append(Song(date.fromisoformat(song_date), *song_data))
...
def random_seat():
    if random.randint(0, 100) < 5:
        if (rand := random.randint(0, 4)) == 0:
            return "你抽到的位子是保羅的大腿上"
        elif rand == 1:
            return "你抽到了大和西大寺站瞭望台"
        elif rand == 2:
            return "你抽到了近鉄奈良線快速急行"
        else:
            return "你抽到了海軍陸戰隊！！"
    seat = random.randint(1, 30001)
    if seat < 4001:
        block = chr(ord('A') + seat // 1000)
        num = seat % 1000
        return f"你抽到的位子是 Premium Area {block} Block {num} 番"
    else:
        seat = seat - 4000
        block = chr(ord('A') + seat // 1000)
        num = seat % 1000
        return f"你抽到的位子是 Standard Area {block} Block {num} 番"
